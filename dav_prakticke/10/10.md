...
In general, SQLite has a global write lock, so is likely to perform poorly when doing parallel data modification. The lack of overhead makes it faster than PostgreSQL for single-threaded insertion of smaller data sets. PostgreSQL also has a faster non-SQL way of importing or exporting called [COPY](https://www.postgresql.org/docs/current/sql-copy.html).
