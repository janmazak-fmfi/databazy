...
1. Vytvorte v jazyku SQL funkciu `calculate_dept_payroll(dept_id)` s jedným argumentom, ktorá pre dané oddelenie vypočíta celkové náklady na mzdy zamestnancov v tomto oddelení. Vyskúšajte ju v dotaze: `SELECT calculate_dept_payroll(...)`.

2. Doplňte do tabuľky oddelení stĺpec `payroll` pre evidenciu celkových nákladov na mzdy za toto oddelenie.

3. (1 point) Vytvorte trigger, ktorý pri pridaní nového zamestnanca príslušne upraví `payroll` za jeho oddelenie v tabuľke oddelení pomocou volania `calculate_dept_payroll` (neefektivitu teraz neriešime). Začať môžete vytvorením vhodnej funkcie v jazyku PL/pgSQL. Inšpirovať sa môžete príkladmi:
...
* task 3 in the section on triggers (includes tasks 1 and 2)
