...
1 point is awarded for a query plan from the first part, i.e. 'Query plans (EXPLAIN / ANALYZE)'. Choose one that you find most interesting / most surprising, and thoroughly explain your reasons for choosing it. In your commentary, don't forget to include the context, like which query it is solving, what indices exists (and might affect the query computation), and how much data is stored in the tables.
