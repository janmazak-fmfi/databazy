...
\begin{frame}{ERM: konceptuálna úroveň (Conceptual Level)}
\begin{itemize}
    \item Identifikuje hlavné entity (napr. Čitateľ, Kniha).
    \item Definuje základné vzťahy medzi entitami (napr. \textit{požičiava~si}).
    \item Neobsahuje atribúty, dátové typy ani kľúče.
    \item Slúži na (úvodnú) komunikáciu s \uv{biznis používateľmi} (manažment, klient).
\end{itemize}
\end{frame}

\begin{frame}{ERM: logická úroveň (Logical Level)}
Detailný návrh štruktúry dát nezávislý od DBMS.
\begin{itemize}
    \item Entity sa stávajú tabuľkami.
    \item Definuje všetky atribúty (stĺpce) pre každú tabuľku.
    \item Určuje primárne kľúče (PK) a cudzie kľúče (FK).
    \item Špecifikuje kardinalitu vzťahov (1:N, M:N).
    \item Stále nerieši špecifické dátové typy (napr. \texttt{VARCHAR} vs \texttt{TEXT}).
\end{itemize}
\end{frame}

\begin{frame}{ERM: fyzická úroveň (Physical Level)}
Konkrétna implementácia pre špecifický databázový systém (DBMS).
\begin{itemize}
    \item Presné názvy tabuliek a stĺpcov.
    \item Definuje špecifické dátové typy (napr. \texttt{TEXT}, \texttt{DATE}).
    \item Zahŕňa obmedzenia (constraints) ako \texttt{NOT NULL}, \texttt{UNIQUE}.
    \item Rieši optimalizáciu výkonu: indexy, partície.
    \item Výsledkom je DDL skript (napr. \texttt{CREATE TABLE...}).
\end{itemize}
\end{frame}
...
\item Vzťah medzi entitami zobrazíme neorientovanou hranou medzi obdĺžnikmi, ku ktorému pripíšeme,
či ide o one-to-one (1:1), one-to-many (1:N), many-to-many (M:N)
...
\begin{frame}[fragile]{Funkčné závislosti}
Príklad: neplatí $X\fz Y$, ale môže platiť $Y\fz X$:\\[3mm]
\centerline{
\begin{tabular}{|c|c|}
    \hline
    X & Y \\
...
($C\fz B$, $AC\fz B$\dots) určite neplatia?\\[5mm]
\centerline{
\begin{tabular}{|c|c|c|}
    \hline
    A & B & C\\
...
\begin{frame}[fragile]{Funkčné závislosti}
Pridanie funkčnej závislosti spraví z relácie funkciu. Táto funkcia je definovaná naplnením tabuľky (nie matematicky ---
to by sme pravú stranu FZ vôbec nemuseli evidovať v databáze, ak by sa dala vždy vypočítať z hodnoty naľavo).
\\[5mm]

Pre tabuľku s dvomi atribútmi $x$, $y$ to presne zodpovedá matematickým pojmom:
\emph{binárna relácia} reprezentuje \emph{funkciu}, ak pre každú hodnotu $x$ máme jedinú hodnotu $y$.
(Technicky, ide o čiastočnú funkciu, nemusí byť definovaná na všetkých prípustných hodnotách $x$,
jej definičný obor je daný konkrétnym naplnením tabuľky.)
\end{frame}
...
O &\fz Z\\
OP &\fz M\\
P &\fz C\quad ??? \quad (\text{zľavy pre veľkoodberateľov?})\\
O &\fz F\quad ??? \quad (\text{čo s opravnými faktúrami?})
\end{align*}

\bigskip
Ak chceme nájsť všetky platné FZ, môžeme postupne pre každý atribút $A$ hľadať minimálne množiny $X$ také, že $X\fz A$.
\end{frame}
...
\begin{frame}[fragile]{Reprezentácia FZ v SQL: \alert{UNIQUE}}
...
\bigskip
Keby sme mali v jednej tabuľke veľa FZ, ich vynucovanie pomocou UNIQUE môže navzájom kolidovať.
To tiež naznačuje vhodnosť rozdelenia veľkej tabuľky na malé (dekompozícia relácie).
\end{frame}
...
Príklad: relácia $order\_items(Order, Product, Amount)$ s~FZ
...
V~praxi sa možno zaobísť aj bez znalosti teórie o nich.
\end{frame}
...
\begin{frame}[fragile]{Normalizácia}
Uvažujme reláciu R(Student, Course, Teacher). Vieme, že:
\begin{itemize}
\item Študent môže študovať viacero predmetov.
\item V rámci jedného predmetu má študent jedného učiteľa.
\item Predmet môže mať viac učiteľov,\\ avšak každý učiteľ učí len jeden predmet.
\end{itemize}

\bigskip
\begin{enumerate}
\item Nájdite všetky platné netriviálne FZ.
\item Nájdite všetky kľúče tejto relácie.
\item Zistite najvyššiu normálnu formu, v ktorej je táto relácia.
\item Bezstratovo ju dekomponujte do BCNF, ak treba.\\
...
Bežné chyby:
\begin{itemize}
\item Namiesto uzáveru množiny všetkých FZ pracujeme len s FZ v konkrétnej reprezentácii tohto uzáveru (napr. niektoré minimálne pokrytie).
...
\begin{frame}[fragile]{Normalizácia}
V relácii s atribútmi id\blue{O}bjednávky, id\blue{Z}ákazníka, id\blue{P}roduktu, \blue{C}ena, \blue{M}nožstvo platia FZ
\begin{eqnarray*}
O\fz Z\\
OP\fz M\\
P\fz C
...
\begin{enumerate}
\item Nájdite všetky kľúče tejto relácie.
\item Dekomponujte ju do 3NF, prípadne BCNF.
\end{enumerate}
\pause
\medskip
Všimnite si, ako dekompozícia smeruje k jednej tabuľke pre každú FZ (ktorá sa ľahko vynúti v SQL pomocou UNIQUE).
\end{frame}

\begin{frame}[fragile]{Normalizácia}
V relácii s atribútmi id\alert{F}aktúry, id\blue{O}bjednávky, id\blue{Z}ákazníka, id\blue{P}roduktu, \blue{C}ena, \blue{M}nožstvo platia FZ
\begin{eqnarray*}
O\fz Z, \quad OP\fz M, \quad P\fz C, \quad \alert{F\fz O}
\end{eqnarray*}
Pridanie faktúry vytvorí problémy, ktoré sa nedajú riešiť len cez FZ. Dva rôzne prípady:\\[2mm]
A. Produkty z objednávky sa rozdelia do viacerých faktúr, každý na presne jednej faktúre.\\[2mm]
B. Faktúra sa vždy vzťahuje na všetky produkty objednávky, ale môže ich byť viac
(chybne vystavené $F$, ktoré už evidujú zákazníci či daňový úrad, preto ich nemožno zmazať).\\[2mm]
\end{frame}
...
O\fz Z, \quad OP\fz M, \quad P\fz C, \quad \alert{F\fz O}
\end{eqnarray*}
V oboch prípadoch dáva zmysel oddeliť $OZ$, $PC$ a $OPM$.
\bigskip

Rozdiel sa prejaví pri dekompozícii $FOP$. Tá nie je v 3NF, lebo jediný kľúč je $FP$ a platí $F\fz O$, preto s ňou v oboch prípadoch potrebujeme niečo spraviť.
\bigskip

Pre $FOP$ prichádza do úvahy jediná bezstratová dekompozícia $FP$, $FO$ (prečo?).
\end{frame}

\begin{frame}[fragile]{Normalizácia}
\begin{small}
A. Produkty z objednávky sa rozdelia do viacerých faktúr, každý na presne jednej faktúre.
\bigskip

Platí tu FZ $OP\fz F$, ktorú stratíme pri dekompozícii $FP$, $FO$.
Zároveň však táto nová FZ zabezpečí, že $FOP$ je aspoň v 3NF, hoci sa nedá bezstratovo dekomponovať do BCNF.
\bigskip

To však nie je všetko. $OP\fz F$ zaručí len to, že faktúra k objednanému produktu je najviac jedna,
nie to, že každý produkt je vyfakturovaný. Potrebujeme teda navyše nejakú rovnosť množín produktov,
a to v SQL nemožno zabezpečiť jednoduchými prostriedkami.
Nevyhneme sa tak použitiu triggerov.
\end{small}
\bigskip

{\scriptsize V praxi je asi jednoduchšie rozhodnúť sa, že nebudeme deliť produkty do viacerých faktúr,
lebo to nemá významnú pridanú hodnotu a zvyšuje riziko chýb (jednoduchšie je rozdeliť objednávku a pôvodnú zmazať).}
\end{frame}

\begin{frame}[fragile]{Normalizácia}
B. Faktúra sa vždy vzťahuje na všetky produkty objednávky.
\bigskip

Dekompozícia $FOP$ do $FP$, $FO$ vyriešila všetky problémy, ktoré sa dajú popísať cez FZ.
Lenže čomu v realite zodpovedá \alert{$FP$}?
\bigskip

Problém: K $F$ je jedna $O$, a na tej sú $P$. Vzťah medzi $F$ a $P$ je tým úplne určený a nemusíme ho evidovať v tabuľke $FP$.
Tabuľku $FP$ môžeme úplne zahodiť, lebo je redundantná: ak obsahuje korektné hodnoty, nemá vplyv na výsledok joinu $FO$ a $OPM$.
\bigskip

To však FZ \uv{nevidia}. V tabuľke $FOP$ platí tzv. multizávislosť $F \twoheadrightarrow P$: množina $P$ k fixnej $F$ je vždy rovnaká bez ohľadu na hodnotu $O$.
(FZ je špeciálnym prípadom multizávislosti s jednoprvkovou množinou.)
\end{frame}

\begin{frame}[fragile]{Vyššie normálne formy (informatívne)}

Niektoré druhy redundancie sa nedajú zachytiť FZ.

Príklad: R = employee(emp\_id, project, skill).
\begin{itemize}
    \item Zamestnanec pracuje na viacerých projektoch.
    \item Zamestnanec má viacero zručností.
    \item Projekty a zručnosti zamestnanca \alert{sú na sebe nezávislé}.
\end{itemize}

Problém (redundancia): Ak Anna pracuje na 7 projektoch a vie 'SQL' a 'Python', tabuľka \alert{musí} obsahovať všetkých 14 riadkov karteziánskeho súčinu.
Vznikajú anomálie: pridanie novej zručnosti pre Annu vyžaduje pridanie 7 nových záznamov.

\medskip
R je v BCNF, lebo neplatia žiadne netriviálne FZ (kľúčom je celá trojica \texttt{emp\_id, project, skill}). BCNF však túto redundanciu neodstránila.
\end{frame}

\begin{frame}[fragile]{4NF a 5NF (informatívne)}
\alert{Multivalued dependencies} (MVD) v relácii $XYZ$:
$X \twoheadrightarrow Y$, ak pre danú hodnotu $X$ existuje \emph{množina} hodnôt $Y$, ktorá je rovnaká pre každú hodnotu $Z$.

Pre R:
$ emp\_id \twoheadrightarrow project$, $emp\_id \twoheadrightarrow skill$.

\bigskip
Relácia je v 4NF, ak v každej netriviálnej MVD $X \twoheadrightarrow Y$ je $X$ je nadkľúč.

\bigskip
MVD odstraňujeme tiež dekompozíciou, napr. $R$ nahradíme $R_1 = (emp\_id, project)$ a $R_2 = (emp\_id, skill)$.
Zbytočnosť tabuľky FP ukáže až 5NF.
\end{frame}
...
\item NO ACTION -- toto je default, DELETE skončí s chybou pri COMMIT (ak DBMS podporuje odloženú kontrolu, inak ihneď)
\end{itemize}
\end{frame}
...
\item \alert{RESTRICT} -- zabráni zmene v odkazovanej tabuľke, UPDATE ihneď skončí s chybou
\item SET NULL -- nastaví odkaz na NULL (väzba zanikne)
\item SET DEFAULT -- nastaví hodnotu odkazu na defaultnú
(zlyhá, ak takto vznikne nekorektný odkaz)
\item NO ACTION -- toto je default, UPDATE skončí s chybou pri COMMIT (ak DBMS podporuje odloženú kontrolu, inak ihneď)
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Nájdite príklad dekompozície, ktorá je bezstratová, ale nezachováva FZ.
\item Nájdite príklad dekompozície, ktorá nie je bezstratová, ale zachováva všetky FZ (pričom platia aspoň dve).
\item Ako možno vynútiť platnosť funkčnej závislosti $AB\fz C$ v bezstratovej dekompozícii,
    ak žiadna relácia neobsahuje zároveň všetky tri atribúty $A$, $B$, $C$?
\item Čo znamená \uv{zachovať FZ pri dekompozícii}? Je nutné zachovať každú FZ? Je to výhodné?
\item Aj dobre napísaný trigger má nevýhody. Aké?
\end{itemize}
\end{frame}

\begin{frame}{Na zamyslenie}
\begin{itemize}
\item Akú časovú zložitosť má vyhodnotenie cudzieho kľúča pri vložení jedného záznamu? Majú na to vplyv indexy?
\item Akú časovú zložitosť má CASCADE pri cudzích kľúčoch? Majú na to vplyv indexy?
\item Uveďte príklad redundancie, ktorá sa nedá vyjadriť pomocou FZ.
\item V prípade tabuľky, ktorá má veľa atribútov, ktorých hodnoty sú takmer všetky rovnaké (napr. NULL, FALSE či default),
    môže mať význam nahradiť ich všetky jediným stĺpcom typu json. V čom spočíva výhodnosť tohto postupu?
    Aká by bola normálna forma takto modifikovanej tabuľky?
\end{itemize}
\end{frame}
