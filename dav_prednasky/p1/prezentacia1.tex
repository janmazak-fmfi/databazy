...
\DeclareMathOperator{\predava}{predava}
\DeclareMathOperator{\navstivil}{navstivil}
...
\item prednášky niekedy zahŕňajú aj \uv{teoretické precvičenie}
\pause
...
\item 3-tier: bežné webové aplikácie: frontend a backend, používateľ nekomunikuje priamo s db a všetka komunikácia ide cez API backendu\\
	(možno napr. filtrovať dáta podľa prihláseného používateľa nad rámec databázového dotazu)
\end{itemize}
\end{frame}
...
Nápoje, ktoré ľúbi každý konzument (ktorý niečo ľúbi), predávajú ich v~každom podniku (kde niečo predávajú) a niekto ich už pil.

{\scriptsize
\begin{align*}
\Big\{N\mid (\exists I\,\exists M\vypil(I, N, M))&\land \forall K \Big( (\exists N_2\lubi(K, N_2))\implies \lubi(K, N)\Big)\\
&\land \forall P \Big((\exists N_2 \predava(P, N_2))\implies \predava(P, N)\Big)\Big\}
\end{align*}
}
{\scriptsize
\begin{align*}
\operatorname{nelubenyNiekym}(N) &\leftarrow \vypil(\_, N, \_), \lubi(K, \_), \lnot\lubi(K, N).\\
\operatorname{nepredavanyNiekde}(N) &\leftarrow \vypil(\_, N, \_), \predava(P, \_), \lnot\predava(P, N).\\
\answer(N) &\leftarrow \vypil(\_, N, \_), \lnot\operatorname{nelubenyNiekym}(N), \lnot\operatorname{nepredavanyNiekde}(N).\\
\end{align*}
}
\vskip -2mm
\vskip -2mm
{\scriptsize
\begin{align*}
\operatorname{nelubenyNiekym} & = (\pi_N(\vypil)\times \pi_K(\lubi))\antijoin \lubi\\
\operatorname{nepredavanyNiekde} & = (\pi_N(\vypil)\times \pi_P(\predava))\antijoin \predava\\
\answer & = ((\pi_N(\vypil) \antijoin \operatorname{nelubenyNiekym})\antijoin \operatorname{nepredavanyNiekde}\\
\end{align*}
}
\end{frame}
...
{\scriptsize
\begin{align*}
\Big\{N\mid (\exists I\,\exists M\vypil(I, N, M))&\land \lnot \Big[\exists K (\exists N_2\lubi(K, N_2))\land\lnot\lubi(K, N)\Big]\Big]\\
&\land \lnot\Big[\exists P \Big((\exists N_2 \predava(P, N_2))\land \lnot\predava(P, N)\Big)\Big]\Big\}
\end{align*}
}
\vskip -2mm
{\scriptsize
\begin{verbatim}
    ...
    WHERE NOT EXISTS (SELECT 1 FROM lubi l
                      WHERE NOT EXISTS (SELECT 1 FROM lubi l2
                                        WHERE l2.K = l.K AND l2.N = v.N))
        AND NOT EXISTS (SELECT 1 FROM predava p
                        WHERE NOT EXISTS (SELECT 1 FROM predava p2
                                        WHERE p2.P = p.P AND p2.N = v.N))
\end{verbatim}
}
\end{frame}
...
\begin{theorem}[CAP theorem for distrib. systems, 2002]
...
\begin{frame}{Databases used by professionals (2025)}
\includegraphics[scale=.26]{commondb2025_modified.png}\\
{\tiny \url{https://survey.stackoverflow.co/2025/}}\\[3mm]
{\small yellow --- relational databases (SQL)}\\
{\small red --- not relational, NoSQL}
\end{frame}
