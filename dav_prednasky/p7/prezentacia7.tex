...
\item \alert{bitmap index scan} --- skenovaním indexu vytvoríme bitmapu popisujúcu bloky z heap file, ktoré treba prečítať, a potom ich načítame celé naraz, nie jednotlivo pre jednotlivé záznamy
\end{itemize}
\end{frame}
...
PostgreSQL automaticky vytvára kvôli primárnemu kľúču index \verb|department_pkey| (aby zabezpečil unikátnosť hodnoty \verb|dept_id|).
\end{frame}
...
pri niektorých operáciách, napr. usporadúvaní, vieme výpočet urýchliť za cenu zvýšenia pamäťových nárokov.)
\end{frame}
